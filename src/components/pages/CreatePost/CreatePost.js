...
const CreatePost = () => {
  ...
  const [publishAt, setPublishAt] = useState(undefined);
  ...
  return (
    <div className="mt-4 mx-10 tracking-wide text-gray-600">
      ...
      <div className="w-1/5 inline-block align-top px-3">
        ...
        <div className="py-2">
          ...
          <div className="grid grid-cols-5 gap-2">
            <input
              ...
              defaultValue={`${moment(n).format("YYYY-MM-DD")}`}
            />
            <input
              type="time"
              min={`${moment(n).format("HH-mm")}`}
              defaultValue={`${moment(n).format("HH:mm")}`}
              onChange={""}
          ...
          {modal.cathegory && (
            <div className="relative my-2 animate-fade-in-down">
              <select
                type="text"
                value={inputValue}
                defaultValue={"Select Cathegory"}
                onChange={(e) => setInputValue(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter") {
                    setCathegory((pre) => [...pre, inputValue]);
                  }
                }}
                className="form-multiselect rounded inline-block border-2 w-full "
              >
              ...
              <span className="absolute right-0 top-0 h-full flex items-center mr-2">
                <svg
                  ...
                  stroke="currentColor"
                  onClick={() => {
                    if (cathegory.includes(inputValue)) return;
                    setCathegory((pre) => {
                      if (pre.includes(inputValue)) return pre;
        ...
        <div>
          <div className="text-gray-500 font-medium py-1.5">Tag</div>
          <div className="relative border-2 rounded-lg bg-white py-2.5 px-2">
            <span className="bg-white text-sm py-1.5 px-2 rounded-lg font-medium bg-gray-200 text-gray-500">
              Technology
            ...
            <span className="absolute right-0 top-0 mr-2 rounded-full h-full flex items-center">
              <svg
                xmlns="http://www.w3.org/2000/svg"
